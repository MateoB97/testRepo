...
export default {
  ...
  methods: {
    ...
    addProducto () {
      ...
      } else if (parseInt(app.precio_producto) === 0) {
        ...
        if (app.listadoPrecios.length === 0) {
          ...
          if (parseInt(app.producto_selected.unidades) === 1) {
            ...
            newProduct = {
              ...
              producto_id: app.producto_selected.id,
              producto_codigo: productoImpuesto.codigo,
              cantidad: app.temp.cantidad_unid,
              precio: parseInt(app.precio_producto) / (1 + (parseInt(productoImpuesto.impuesto) / 100)),
              iva: productoImpuesto.impuesto,
              gen_iva_id: productoImpuesto.gen_iva_id,
              desc: 0.00,
            ...
            axios.get(app.$store.state.jhsoft.url + 'api/facturacion/editarfactura/' + app.$route.params.id + '/' + app.$route.params.consecmov).then(
              function (response2) {
                if (response2.data.lineas) {
                  ...
                  // cargar tercero
                  app.sucursal = response2.data.sucursal
                  app.storeItems.fecha_facturacion = response2.data.mov.fecha_facturacion
                  app.storeItems.fecha_vencimiento = response2.data.mov.fecha_vencimiento
                  if (response2.data.mov.nota !== null) {
                    app.storeItems.nota = response2.data.mov.nota
                  } else {
                    delete app.storeItems.nota
                  }
                  if (response2.data.mov.prod_grupo_id !== null) {
                    app.storeItems.prod_grupo_id = app.grupos.find(v => parseInt(v.id) === parseInt(response2.data.mov.prod_grupo_id))
                  } else {
                    delete app.storeItems.prod_grupo_id
                  }
                  app.pagos = response2.data.pagos
                  app.storeItems.gen_vendedor_id = app.vendedores.find(v => parseInt(v.codigo_unico) === parseInt(response2.data.vendedor))
                } else {
