  ...
  <div>
    <q-page padding>
      <div class="div-86">
              ...
              <q-page padding>
                <div class="overflow-hidden">
                  <div class="row q-col-gutter-sm">
                    <h3>Se ha guardado la {{ tipoComproEgreso.nombre }} N° {{ callback[0] }}</h3>
                  </div>
                  <div class="row text-center">
                    <a target="_blank" :href="$store.state.jhsoft.url+'api/compras/comprobanteegreso/imprimir/'+ callback[1] +'?token='+ $store.state.jhsoft.token"><q-btn class="q-ml-xs" icon="assignment" color="primary"></q-btn> </a>
        ...
        <h4 style="margin: 15px 0px">{{ tipoComproEgreso.nombre }}</h4>
        <div class="row q-col-gutter-md">
            <div class="col-8">
                <SelectTerceroSucursal v-model="sucursal_id" :editor="sucursal_id" labelTercero='Cliente'/>
            </div>
            <div class="col-4">
              <q-input label="Fecha de Comprobante" v-model="storeItems.fecha_comprobante" mask="date" class="date-field" :rules="['date']">
                <template v-slot:append>
                  <q-icon name="event" class="cursor-pointer">
                    <q-popup-proxy ref="qDateProxy" transition-show="scale" transition-hide="scale">
                      <q-date v-model="storeItems.fecha_comprobante" @input="() => $refs.qDateProxy.hide()" />
                    </q-popup-proxy>
                  </q-icon>
                </template>
              </q-input>
        ...
        <div class="row q-col-gutter-md q-mt-md text-right">
          ...
          <!-- <div class="col-4">
            ...
            <div class="col-12 q-mt-md">
              <q-btn color="primary" v-on:click="validacion()" label="Guardar" />
            </div>
          </div>
          <div class="col-8">
                <div style="width:100%">
...
export default {
  ...
  methods: {
    postSave (callback) {
      ...
      today = yyyy + '/' + mm + '/' + dd
      this.storeItems.fecha_comprobante = today
    },
    preSave () {
      ...
      this.storeItems.com_tipo_compro_egresos_id = this.tipoComproEgreso.id
      this.storeItems.proveedor_id = this.sucursal_id
    },
    postEdit () {
    },
    validacion () {
  ...
  watch: {
    ...
    $route: {
      ...
      handler () {
        var app = this
        this.sucursal_id = null
        axios.get(this.$store.state.jhsoft.url + 'api/compras/tiposcomproegreso/' + this.$route.params.id).then(
          function (response) {
            app.tipoComproEgreso = response.data
            app.$q.loading.hide()
          }
        )
      }
