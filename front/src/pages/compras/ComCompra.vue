                  ...
                  <div class="overflow-hidden">
                    <div class="row q-col-gutter-sm">
                      <div class="col-12 text-center">
                        <h4 class="no-margin">¡{{ tipoCompra.nombre }} Guardada! </h4>
                      </div>
                      <div class="col-12 text-center">
                    ...
                    <div class="row col-12 q-col-gutter-md q-mt-md">
                      <div class="col-6 text-center">
                        <a target="_blank" :href="$store.state.jhsoft.url+'api/compras/imprimir/'+ callback[1]+'?token='+ $store.state.jhsoft.token "><q-btn class="q-ml-xs btn-naranja"> Imprimir Carta</q-btn> </a>
                      </div>
                      <div class="col-6 text-center">
        ...
        <!-- inicio popup ingresar pago -->
          <q-dialog tabindex="0" @keyup.enter="localValidate()" v-model="openedAddPago" :content-css="{minWidth: '80vw', minHeight: '50vh'}">
            <q-layout view="Lhh lpR fff" container style="height: 50vh; max-width: 800px" class="bg-white">
              ...
              <q-page-container>
                  <q-page padding>
                  <h3>Agregar Pago - Total: $ {{ total | toMoney }}</h3>
                  <div class="overflow-hidden">
        ...
        <div class="row q-col-gutter-md col-4">
            ...
            <div class="col-12 row q-col-gutter-md">
              ...
              <!-- inicio Datos solo para notas -->
                <div v-if="tipoCompra.naturaleza != '1' && tipoCompra.naturaleza != '4'" class="col-6">
                    <q-select
                    class="w-100"
              ...
              <!-- // -->
              <div class="col-6">
                <q-input label="Fecha de Movimiento" v-model="storeItems.fecha_compra" class="date-field" mask="date" :rules="['date']">
                  <template v-slot:append>
                    <q-icon name="event" class="cursor-pointer">
                      <q-popup-proxy ref="qDateProxy1" transition-show="scale" transition-hide="scale">
                        <q-date v-model="storeItems.fecha_compra" @input="() => $refs.qDateProxy1.hide()" />
                      </q-popup-proxy>
                    </q-icon>
                  </template>
                </q-input>
              ...
              <!-- // -->
              <!-- inicio datos solo para compras -->
                <div class="col-6" v-if="tipoCompra.naturaleza === '1'">
                ...
                <div  v-if="tipoCompra.naturaleza === '1'" class="col-6">
                    <q-input v-model="storeItems.doc_referencia" label="Factura Referencia"/>
                </div>
                <div class="col-6">
            ...
            <div class="row col-12 q-col-gutter-sm box-resumen-movimiento q-mt-md">
              ...
              <div class="col-6">
                <div v-if="tipoCompra.naturaleza != 0" class="col-12">
                ...
                <div class="col-12 q-mt-md">
                  ...
                  >
                    <template v-slot:body="props">
                      <q-tr :props="props">
                        <q-td key="producto_id" :props="props"><q-checkbox v-if="tipoCompra.naturaleza != 0" v-model="selected" :val="props.row.producto_id" /></q-td>
                        ...
                        <q-td key="desc" :props="props">
                          ...
                          <q-popup-edit v-if="tipoCompra.naturaleza != 0" v-model="props.row.desc" @hide="setPorcentaje(props.row.id)" title="Descuento ($)" buttons>
                            <money v-model="props.row.desc" v-bind="money" class="v-money"></money>
                          </q-popup-edit>
                        </q-td>
                        <q-td key="descporcentaje" :props="props">
                          {{ props.row.descporcentaje }}
                          <q-popup-edit v-if="tipoCompra.naturaleza != 0" v-model="props.row.descporcentaje" @hide="setDescuento(props.row.id)" title="Descuento (%)" buttons>
                        ...
                        <q-td key="iva" :props="props">
                          {{ props.row.iva }}
                          <q-popup-edit v-if="tipoCompra.naturaleza != 0" v-model="props.row.iva" title="iva" buttons>
                        ...
                        <q-td key="preciototalunit" :props="props">
                          ...
                          <q-popup-edit v-if="tipoCompra.naturaleza != 0" v-model="temp.precioLinea" @hide="setPrecio(props.row.id)" title="Precio Total Unit." buttons>
                  ...
                  <div v-if="tipoCompra.naturaleza != 0" class="col-12 q-mt-md text-center">
...
export default {
  ...
  methods: {
    postSave (callback = null) {
      ...
      today = yyyy + '/' + mm + '/' + dd
      this.storeItems.fecha_compra = today
      this.storeItems.fecha_vencimiento = today
    },
    preSave () {
      if (this.tipoCompra.naturaleza === '1') {
        this.storeItems.lineas = this.dataResumen
        this.storeItems.proveedor_id = this.sucursal
    ...
    filterCompras (val, update, abort) {
      update(() => {
        const needle = val.toLowerCase()
        this.options.compras = this.compras.filter(v => v.consecutivo.toLowerCase().indexOf(needle) > -1)
      })
    },
    ...
    selectCompra () {
      var app = this
      axios.get(app.$store.state.jhsoft.url + 'api/compras/comprasitems/filtro/porcompra/' + this.storeItems.docReferencia.id).then(
        function (response) {
          app.dataResumen = response.data
          app.dataResumen.forEach(function (element, i) {
            element.desc = ((parseInt(element.precio) / ((parseInt(element.iva) / 100) + 1)) * (parseInt(element.descporcentaje) / 100)).toFixed(2)
            element.descporcentaje = parseInt(element.descporcentaje)
    ...
    verificarTipoCompra () {
      ...
      this.sucursal = null
      var app = this
      axios.get(this.$store.state.jhsoft.url + 'api/compras/tipos/' + this.$route.params.id).then(
        function (response) {
          app.tipoCompra = response.data
          app.dataResumen = []
          app.pagos = []
          if (response.data.naturaleza === '0') {
            axios.get(app.$store.state.jhsoft.url + 'api/compras/movrelacionado/' + response.data.id + '/items').then(
              function (response1) {
                app.compras = response1.data
    ...
    printPOS (id) {
      var app = this
      axios.get(app.$store.state.jhsoft.url + 'api/compras/imprimirpos/' + id).then(
        function (response) {
  ...
  created: function () {
    ...
    var mm = String(today.getMonth() + 1).padStart(2, '0')
    var yyyy = today.getFullYear()
    this.storeItems.fecha_compra = yyyy + '/' + mm + '/' + dd
    var today2 = new Date()
